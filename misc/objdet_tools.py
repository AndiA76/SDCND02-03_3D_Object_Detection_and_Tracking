...
def project_detections_into_bev(bev_map, detections, configs, color=[]):
    for row in detections:
        ...
        cv2.line(
            bev_map,
            (corners_int[0, 0], corners_int[0, 1]), (corners_int[3, 0], corners_int[3, 1]),
            (255, 255, 0),
            2
        )
...
# compute bounding box corner locations [front_left, rear_left, rear_right, front_right]
def compute_box_corners(x, y, w, l, yaw):
    cos_yaw = np.cos(yaw)
    sin_yaw = np.sin(yaw)

    front_left = (x - w / 2 * cos_yaw - l / 2 * sin_yaw,
          y - w / 2 * sin_yaw + l / 2 * cos_yaw)

    rear_left = (x - w / 2 * cos_yaw + l / 2 * sin_yaw,
          y - w / 2 * sin_yaw - l / 2 * cos_yaw)

    rear_right = (x + w / 2 * cos_yaw + l / 2 * sin_yaw,
          y + w / 2 * sin_yaw - l / 2 * cos_yaw)

    front_right = (x + w / 2 * cos_yaw - l / 2 * sin_yaw,
          y + w / 2 * sin_yaw + l / 2 * cos_yaw)

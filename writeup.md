...
The global nearest neighbor (GNN) track-measurement association has been implemented and tested against the same image sequences. Figure 5.3.3 shows some results. The sum of track scores over all tracks is a little bit smaller for the global nearest neighbor than for the single nearest neighbor approach. The reason may be that the GNN reaches a better overall association than the SNN in this example.
...
*Fig. 5.4.2: Tracking of multiple target objects over time on sequence 1 (1005081002024129653_5313_150_5333_150) for the frame ids 0...198 using extended Kalman filters and camera-lidar sensor fusion with gating and single nearest neighbor association based on smallest Mahalanobis distance.*
